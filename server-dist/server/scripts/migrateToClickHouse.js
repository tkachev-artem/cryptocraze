...
exports.migrateDealsToClickHouse = migrateDealsToClickHouse;
exports.migrateUsersToClickHouse = migrateUsersToClickHouse;
...
async function migrateDealsToClickHouse() {
    ...
    try {
        ...
        const allDeals = await db_js_1.db.select().from(schema_1.deals);
        ...
        for (let i = 0; i < allDeals.length; i += batchSize) {
            ...
            for (const deal of batch) {
                try {
                    ...
                    await clickhouseAnalyticsService_js_1.clickhouseAnalyticsService.syncDeal(clickHouseDeal);
                    migratedCount++;
        ...
        // Проверим результат
        const stats = await clickhouseAnalyticsService_js_1.clickhouseAnalyticsService.getDashboardOverview();
...
async function migrateUsersToClickHouse() {
    ...
    try {
        ...
        const allUsers = await db_js_1.db.select().from(schema_1.users);
        console.log(`[Migration] Найдено ${allUsers.length} пользователей`);
        let migratedCount = 0;
        for (const user of allUsers) {
            try {
                // Создаем базовое событие входа для каждого пользователя
                await clickhouseAnalyticsService_js_1.clickhouseAnalyticsService.logUserEvent(parseInt(user.id), 'migration_user', {
