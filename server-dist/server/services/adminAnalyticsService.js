...
exports.adminAnalyticsService = exports.AdminAnalyticsService = void 0;
...
class AdminAnalyticsService {
    ...
    async getOverview() {
        try {
            ...
            const [engagementResult, revenueResult, acquisitionResult, usersResult, dealsResult] = await Promise.all([
        ...
        `),
                ...
                // Активные сделки
                db_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT COUNT(*) as count FROM deals WHERE status = 'open'`)
            ]);
            // Безопасное извлечение данных с проверкой наличия
            const engagement = engagementResult.rows?.[0] || null;
            ...
            const activeDeals = Number(dealsResult.rows?.[0]?.count || 0);
    ...
    async getAdsData(days = 30) {
      ...
      `);
            ...
            const summaryResult = await db_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT 
     ...
     */
    safeMoneyString(value, defaultValue = '0.00') {
...
exports.adminAnalyticsService = new AdminAnalyticsService();
