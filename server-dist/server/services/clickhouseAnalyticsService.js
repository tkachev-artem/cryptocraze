...
exports.clickhouseAnalyticsService = exports.ClickHouseAnalyticsService = void 0;
const clickhouseClient_js_1 = require("./clickhouseClient.js");
...
class ClickHouseAnalyticsService {
    client = (0, clickhouseClient_js_1.getClickHouseClient)();
    /**
      ...
      TTL date + INTERVAL 1 YEAR`
        ...
        for (const index of indexes) {
            try {
                ...
                console.log('[ClickHouse] Index creation note:', error.message);
            }
        }
    }
    /**
     ...
     */
    async logUserEvent(userId, eventType, eventData = {}, sessionId) {
        const eventRecord = {
     ...
     */
    async logRevenueEvent(userId, revenueType, amount, currency = 'USD', paymentMethod, subscriptionId) {
        try {
            await this.client.insert({
    ...
    async getDashboardOverview() {
        try {
            ...
            const [userMetrics, tradingMetrics, revenueMetrics, engagementMetrics] = await Promise.all([
                this.getUserMetrics(),
    ...
    async getUserMetrics() {
        ...
        `,
            ...
            const retentionData = (await retentionResult.json())[0] || {};
    ...
    async getRevenueMetrics() {
        ...
        `,
            ...
            const data = await revenueResult.json();
    ...
    async getEngagementMetrics() {
      ...
      `,
        ...
        const mainData = (await mainResult.json())[0] || {};
        const sessionData = (await sessionDurationResult.json())[0] || {};
        const tutorialData = (await tutorialResult.json())[0] || {};
     ...
     */
    async getTimeSeriesData(metric, days = 7) {
        try {
            let query = '';
            switch (metric) {
                case 'users':
...
exports.ClickHouseAnalyticsService = ClickHouseAnalyticsService;
exports.clickhouseAnalyticsService = new ClickHouseAnalyticsService();
