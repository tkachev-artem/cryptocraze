...
router.get('/queues', requireAdmin, async (req, res) => {
    try {
        const queueStats = await queueManager_js_1.queueManager.getQueueStats();
...
router.get('/prices', requireAdmin, async (req, res) => {
    try {
        const stats = priceMonitorService_js_1.priceMonitorService.getStats();
        const health = priceMonitorService_js_1.priceMonitorService.getHealthStatus();
        const currentPrices = priceMonitorService_js_1.priceMonitorService.getAllCurrentPrices();
        res.json({
            success: true,
            data: {
                stats,
...
router.get('/closures', requireAdmin, async (req, res) => {
    try {
        const stats = orderClosureService_js_1.orderClosureService.getStats();
        const health = orderClosureService_js_1.orderClosureService.getHealthStatus();
        const recentErrors = orderClosureService_js_1.orderClosureService.getRecentErrors();
        res.json({
            success: true,
            data: {
                stats,
 ...
 */
router.post('/shutdown', requireAdmin, async (req, res) => {
    try {
 ...
 */
router.get('/jobs/:dealId', requireAdmin, async (req, res) => {
    try {
        ...
        const job = await queueManager_js_1.queueManager.orderMonitorQueue.getJob(jobId);
        if (!job) {
 ...
 */
router.delete('/jobs/:dealId', requireAdmin, async (req, res) => {
    try {
        const dealId = parseInt(req.params.dealId);
        if (isNaN(dealId)) {
            return res.status(400).json({
        ...
        const removed = await workerManager_js_1.workerManager.removeOrderFromMonitoring(dealId);
        res.json({
            success: true,
            data: {
                dealId,
 ...
 */
router.get('/metrics/export', requireAdmin, async (req, res) => {
    try {
        const health = await workerManager_js_1.workerManager.getSystemHealth();
        const uptime = workerManager_js_1.workerManager.getUptime();
        const recentErrors = orderClosureService_js_1.orderClosureService.getRecentErrors();
 ...
 */
router.get('/monitor/stream', requireAdmin, (req, res) => {
