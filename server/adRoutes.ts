...
import { isAuthenticated, isAdminWithAuth } from "./simpleOAuth";
import { adService, AdServiceError } from './services/adService';
...
export function registerAdRoutes(app: Express): void {
  // ============ AD SYSTEM ROUTES ============
   ...
   */
  app.post('/api/ads/session/start', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const sessionId = await adService.startAdSession(userId, {
        adId,
        placement,
  ...
  app.post('/api/ads/session/complete', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await adService.completeAdSession(userId, {
        adId,
        watchTime,
        reward,
   ...
   *                       format: date-time
   */
  app.get('/api/ads/user/stats', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id ?? req.user?.claims?.sub;
      if (!userId) {
        res.status(401).json({ success: false, message: 'Unauthorized' });
      ...
      const stats = await adService.getUserAdStats(userId);
   ...
   */
  app.get('/api/ads/analytics', isAdminWithAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const timeframe = (req.query.timeframe as string) || 'day';
      const analytics = await adService.getAnalytics(timeframe as 'day' | 'week' | 'month');
   ...
   *                       format: date-time
   */
  app.get('/api/ads/check-eligibility', isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id ?? req.user?.claims?.sub;
      if (!userId) {
        res.status(401).json({ success: false, message: 'Unauthorized' });
