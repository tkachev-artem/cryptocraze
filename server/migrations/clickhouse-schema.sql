-- ClickHouse Schema for CryptoCraze Analytics
-- Высокопроизводительные таблицы для аналитики admin dashboard
...
-- Эти настройки применяются к текущей сессии, но лучше настроить на уровне сервера
...
-- Функция для расчета когорт пользователей
...
COMMENT ON TABLE user_events IS 'Основная таблица событий пользователей для трекинга активности';
COMMENT ON TABLE deals_analytics IS 'Аналитическая копия сделок с дополнительными вычисляемыми полями';
COMMENT ON TABLE users_analytics IS 'Денормализованная таблица пользователей для быстрых аналитических запросов';
COMMENT ON TABLE daily_metrics IS 'Агрегированные ежедневные метрики для dashboard';
COMMENT ON TABLE ad_events IS 'События связанные с рекламой для анализа монетизации';