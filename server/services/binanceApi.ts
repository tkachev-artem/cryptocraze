...
export class BinanceApiService {
  private readonly basePath = '/api/v3';
  ...
  async getCandlestickData(
    ...
    try {
      const cached = await redisService.getCandles(upperSymbol, interval);
    ...
    for (let i = 0; i < this.baseDomains.length; i++) {
      ...
      try {
        ...
        const candlesticks = data.map((candle: any[]) => ({
          openTime: candle[0],
        ...
        await redisService.setCandles(upperSymbol, interval, candlesticks);
        return candlesticks;
      } catch (error) {
