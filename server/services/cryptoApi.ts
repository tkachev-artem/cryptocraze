...
export class CryptoApiService extends EventEmitter {
  ...
  private readonly symbolToId = new Map([
    ['BTCUSDT', 'bitcoin'],
    ['ETHUSDT', 'ethereum'],
    ['ADAUSDT', 'cardano'],
    ['DOTUSDT', 'polkadot'],
  ...
  async getCandlestickData(symbol: string, interval: string, limit: number = 100): Promise<CandlestickData[]> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.API_BASE}/coins/${coinId}/market_chart?vs_currency=usd&days=${days}&interval=${interval}`
      );
  ...
  private async fetchPrice(symbol: string): Promise<PriceData> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.API_BASE}/simple/price?ids=${coinId}&vs_currencies=usd&include_24hr_vol=true&include_24hr_change=true`
      );
  ...
  private async updatePrices(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.API_BASE}/simple/price?ids=${coinIds}&vs_currencies=usd&include_24hr_vol=true&include_24hr_change=true`
      );
