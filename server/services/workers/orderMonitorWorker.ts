...
import { OrderMonitorJobData, queueManager } from './queueManager.js';
 ...
 */
export class OrderMonitorWorker {
  private worker: Worker<OrderMonitorJobData>;
  ...
  constructor() {
    this.worker = new Worker<OrderMonitorJobData>(
      'orderMonitor',
    ...
    queueManager.registerWorker('orderMonitor', this.worker);
   ...
   */
  private async processOrder(job: Job<OrderMonitorJobData>): Promise<void> {
    ...
    try {
      ...
      if (this.isShuttingDown) {
        throw new UnrecoverableError('Worker shutting down');
      }
      ...
      if (error instanceof UnrecoverableError) {
        ...
        await queueManager.moveToDeadLetter(job.data, error.message);
        await job.remove();
        throw error;
      }
      ...
      if (failures >= 10) {
        await queueManager.moveToDeadLetter(job.data, `Too many failures: ${failures}`);
        await job.remove();
        throw new UnrecoverableError(`Order ${dealId} failed too many times`);
      }
   ...
   */
  private async validateOrderStatus(dealId: number): Promise<boolean> {
    try {
   ...
   */
  private async executeOrderClosure(
    orderData: OrderMonitorJobData,
    checkResult: TPSLCheckResult
  ): Promise<void> {
    ...
    try {
      ...
      const result = await dealsService.closeDeal({ userId, dealId });
   ...
   */
  private logOrderProgress(dealId: number, symbol: string, checkResult: TPSLCheckResult): void {
   ...
   */
  private incrementFailedAttempts(dealId: number): void {
...
export const orderMonitorWorker = new OrderMonitorWorker();