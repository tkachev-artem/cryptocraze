...
const MetricTable: React.FC<MetricTableProps> = ({ metricId, title, isOpen, onClose, dateRange, selectedFilters }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (['D1', 'D3', 'D7', 'D30'].includes(metricId)) {
        ...
        endpoint = '/admin/dashboard/table/win_rate';
      } else if (metricId === 'average_profit_loss') {
        endpoint = '/admin/dashboard/table/average_profit_loss';
      } else if (metricId === 'max_profit_trade') {
        endpoint = '/admin/dashboard/table/max_profit_trade';
  ...
  const renderTableContent = () => {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left py-4 px-4 font-bold text-gray-900">Region</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Type</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Total Trades</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Wins</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Losses</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Win Rate</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg P/L</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Last Trade</th>
            </tr>
          </thead>
          <tbody>
            {data.map((row) => (
              <tr key={row.userId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-4 px-4 text-sm text-gray-700">{row.totalTrades}</td>
                <td className="py-4 px-4 text-sm text-green-600 font-medium">{row.wins}</td>
                <td className="py-4 px-4 text-sm text-red-600 font-medium">{row.losses}</td>
                <td className="py-4 px-4 text-sm text-gray-700">{Number(row.winRate ?? 0).toFixed(2)}%</td>
    ...
    // Таблица для Average Profit/Loss
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left py-4 px-4 font-bold text-gray-900">Total Trades</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg P/L</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Median P/L</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg Position Size</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg Leverage</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Last Trade</th>
            </tr>
          </thead>
          <tbody>
            {data.map((row) => (
              <tr key={row.userId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-4 px-4 text-sm text-gray-700">{row.totalTrades}</td>
                <td className={`py-4 px-4 text-sm ${Number(row.avgPnL ?? 0) > 0 ? 'text-green-600' : Number(row.avgPnL ?? 0) < 0 ? 'text-red-600' : 'text-gray-700'}`}>{Number(row.avgPnL ?? 0).toFixed(2)}</td>
                <td className={`py-4 px-4 text-sm ${Number(row.medianPnL ?? 0) > 0 ? 'text-green-600' : Number(row.medianPnL ?? 0) < 0 ? 'text-red-600' : 'text-gray-700'}`}>{Number(row.medianPnL ?? 0).toFixed(2)}</td>
                <td className="py-4 px-4 text-sm text-gray-700">{Number(row.avgPositionSize ?? 0).toFixed(2)}</td>
                <td className="py-4 px-4 text-sm text-gray-700">{Number(row.avgLeverage ?? 0).toFixed(2)}x</td>
                <td className="py-4 px-4 text-sm text-gray-700">{row.lastTradeAt ? new Date(row.lastTradeAt).toUTCString() : '—'}</td>
              </tr>
            ))}
          </tbody>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left py-4 px-4 font-bold text-gray-900">Region</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Type</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Symbol</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Direction</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Amount</th>
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.dealId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="py-4 px-4 text-sm text-gray-900 font-mono">{row.userId}</td>
                <td className="py-4 px-4 text-sm text-gray-700">
                  <div className="flex items-center gap-2">
                    {row.country && row.country !== 'Unknown' && (
                ...
                <td className="py-4 px-4 text-sm text-gray-700">{row.symbol} ({row.leverage}x)</td>
                <td className={`py-4 px-4 text-sm capitalize ${row.direction === 'up' ? 'text-green-600' : 'text-red-600'}`}>{row.direction}</td>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg Holding (min)</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Median Holding (min)</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg P/L</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Avg Position Size</th>
              <th className="text-left py-4 px-4 font-bold text-gray-900">Last Trade</th>
            </tr>
